...
if __name__ == '__main__':
    ...
    for i in tqdm(range(N)):
        ...
        pred = np.fromfile(pred_file, dtype=np.uint32)
        pred = pred.reshape((-1))    # reshape to vector
        sem_pred = pred & 0xFFFF  # semantic label in lower half
        inst_pred = pred >> 16  # instance id in upper half
        assert ((sem_pred + (inst_pred << 16) == pred).all())
        pred = remap_lut[sem_pred]
        if FLAGS.limit is not None:
    ...
    # confusion = evaluator.get_confusion()
    # np.save('/data/zclin/hl/RandLA-Net-pytorch/Experimental-results-baseline/cm.npy',confusion)
    
    print('Validation set:\n'
          'Acc avg {m_accuracy:.3f}\n'
          'IoU avg {m_jaccard:.4f}'.format(m_accuracy=m_accuracy, m_jaccard=m_jaccard))
